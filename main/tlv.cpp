...
vector<TLV> TLV::decode(forward_list<uint8_t> data) {
  ...
  uint8_t current_tlv_type = -1;
  uint8_t current_tlv_size = -1;
  ...
  for(auto &byte : data){
    if(current_tlv_type == -1) {
      ...
      if(possible_fragmentation && prev_tlv_type != current_tlv_type){
        decoded_tlvs.push_back(TLV(prev_tlv_type, current_tlv_value));
      ...
      if(!possible_fragmentation){
        decoded_tlvs.push_back(TLV(current_tlv_type, current_tlv_value));
        current_tlv_value.clear();
      }
